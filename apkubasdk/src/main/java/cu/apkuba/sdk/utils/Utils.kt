 ...
 *
 * This library provides easy integration with Apklis and ApKuba
 * payment verification systems for Android applications.
 *
 * Licensed under the MIT License.
 * See LICENSE file in the project root for full license information.
 *
 * GitHub: https://github.com/megashopcuba/ApKubaSdk
 *
 */

package cu.apkuba.sdk.utils
...
object Utils {
    ...
    private const val APKLIS_COMPANION_APP_ID = "cu.apkuba"
    private const val APKLIS_COMPANION_WEB_URL = "https://www.megashopcuba.com/apps/apkuba"
    ...
    fun openAppLink(context: Context, appPackage: String) {
        val isApKubaInstalled = isAppInstalled(context, APKLIS_COMPANION_APP_ID)
        val isApklisInstalled = isAppInstalled(context, APKLIS_APP_ID)

        if (isApKubaInstalled) {
            openApKubaLink(context, appPackage)
    ...
    fun openApKubaLink(context: Context, appPackage: String): Boolean {
        ...
        val isApKubaInstalled = isAppInstalled(context, APKLIS_COMPANION_APP_ID)
        val fullLink = getApKubaUrl(appPackage)
        return if (isApKubaInstalled && openApkLisPaymentCheckLink(
                context,
                fullLink
            )
        )
            ...
            Logger.e("La app ApKuba no está instalada en el dispositivo")
    ...
    fun openApKubaLicenseLink(
        ...
        return if (isApKubaInstalled && openApkLisPaymentCheckLink(
                context,
                getApKubaLicenseUrl(applicationId, licenseUuid, publicKeyPem)
            )
        )
            true
        else {
    ...
    private fun getApKubaUrl(applicationId: String): String {
        return "${APP_SCHEME}://$applicationId"
    }

    private fun getApKubaLicenseUrl(
        applicationId: String,
        licenseUuid: String,
        publicKeyPem: String
    ): String {
